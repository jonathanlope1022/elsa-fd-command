...
let activeUnit=null,activeMutualAid=null,activeBenchmark=null,assignmentTarget=null,audioCtx=null,alarmInterval=null,alarmStartedAt=null,audioUnlocked=false,timelineFilter="All",playbackEvents=[],playbackIndex=0,playbackTimer=null;
...
async function ensureAudio(){
  try{
    ...
    $("parAudioStatus").textContent=audioUnlocked?"Alarm enabled":"Tap Enable / Test Alarm";
    return audioUnlocked
  }catch(e){
    audioUnlocked=false;$("parAudioStatus").textContent="Audio unavailable";return false
  }
}
function tone(freq=1200,duration=.16,delay=0){
  if(!audioCtx||audioCtx.state!=="running")return;
  const o=audioCtx.createOscillator(),g=audioCtx.createGain();
  o.type="square";o.frequency.setValueAtTime(freq,audioCtx.currentTime+delay);
  g.gain.setValueAtTime(.0001,audioCtx.currentTime+delay);
  g.gain.exponentialRampToValueAtTime(.32,audioCtx.currentTime+delay+.01);
...
async function urgentBurst(){
  await ensureAudio();
  tone(1250,.13,0);tone(1250,.13,.18);tone(1600,.18,.36);
  setTimeout(()=>{tone(1250,.13,0);tone(1250,.13,.18);tone(1600,.18,.36)},650);
  if(navigator.vibrate)navigator.vibrate([250,80,250,80,350,150,350])
}
async function testParAlarm(){
  const ok=await ensureAudio();
  if(!ok)return alert("Audio could not be enabled. Make sure the iPad is not muted and tap again.");
  urgentBurst();$("parAudioStatus").textContent="Alarm enabled and tested"
}
...
function startAlarm(){
  ...
  alarmInterval=setInterval(urgentBurst,1800)
}
function stopAlarm(){

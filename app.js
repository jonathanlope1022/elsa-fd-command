...
"Fire Operations":["Ventilation Started","Ventilation Complete","Overhaul Started","Overhaul Complete","Control Time"],
...
let activeUnit=null,activeBenchmark=null,assignmentTarget=null,audioCtx=null,alarmInterval=null,timelineFilter="All";

function defaults(){
  const units={};APPARATUS.forEach(([n,t,w,m])=>units[n]={type:t,status:"Not Responding",personnel:0,assignment:"",water:w,manualWater:m,par:"Pending"});
...
function init(){
  state=defaults();load();
  bindFields();renderAssignments();renderUnits();renderBenchmarks();renderMutualAid();renderTimeline();renderLog();renderCommandAssignments();updateMetrics();updateTimers();
...
function bindFields(){
  ...
  $("incidentType").addEventListener("change",()=>{$("otherIncidentType").classList.toggle("hidden",$("incidentType").value!=="Other");save(false)});
...
function updateTimers(){
  ...
  if(r<=0){el.textContent="PAR DUE";el.classList.add("par-due");startAlarm()}
...
function beep(){
  ensureAudio();if(!audioCtx)return;const o=audioCtx.createOscillator(),g=audioCtx.createGain();o.frequency.value=880;g.gain.setValueAtTime(.15,audioCtx.currentTime);o.connect(g);g.connect(audioCtx.destination);o.start();o.stop(audioCtx.currentTime+.35);if(navigator.vibrate)navigator.vibrate([250,120,250])
}
function startAlarm(){if(alarmInterval)return;beep();alarmInterval=setInterval(beep,3000)}
...
function renderUnits(){
  $("units").innerHTML="";Object.entries(state.units).forEach(([n,u])=>{
    ...
    d.innerHTML=`<h4>${esc(n)}</h4><p>${esc(u.type)}</p><p>Status: <b>${esc(u.status)}</b></p><p>Assignment: ${esc(u.assignment||"—")}</p><p>Personnel: ${u.personnel}</p><p>Water: ${u.water} gal</p><p>PAR: ${esc(u.par)}</p>`;
    d.onclick=()=>openUnit(n);$("units").appendChild(d)
  });renderCommandAssignments();updateMetrics()
}
function openUnit(n){
...
function saveUnit(){
  const old=state.units[activeUnit],u={...old,status:$("unitStatus").value,personnel:+$("unitPersonnel").value||0,assignment:$("unitAssignment").value,par:$("unitPar").value};
  if(u.manualWater)u.water=+$("unitWater").value||0;state.units[activeUnit]=u;
  let msg=`${activeUnit}: ${u.status}${u.assignment?" — "+u.assignment:""}`;addLog(msg,"Resources");
  if(u.assignment!==old.assignment&&u.assignment==="Fire Attack")addTimeline("Fire Attack Started","Fire Operations",activeUnit);
...
function addMutualAid(){
  ...
  const m={id:crypto.randomUUID(),department:$("maDept").value.trim(),unit:$("maUnit").value.trim(),type:$("maType").value,status:$("maStatus").value,personnel:+$("maPersonnel").value||0,gallons:+$("maGallons").value||0,assignment};
...
function renderMutualAid(){
  $("maBody").innerHTML=state.mutualAid.map(m=>`<tr><td>${esc(m.department)}</td><td>${esc(m.unit)}</td><td>${esc(m.type)}</td><td>${esc(m.status)}</td><td>${m.personnel}</td><td>${m.gallons}</td><td>${esc(m.assignment||"—")}</td><td><button class="btn light small" data-remove-ma="${m.id}">Remove</button></td></tr>`).join("");
  ...
  renderAssignments();renderCommandAssignments();updateMetrics()
}
